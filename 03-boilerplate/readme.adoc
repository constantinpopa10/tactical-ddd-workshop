...
** Finally, replace the `CustomerId customer` property in `OrderTable` with `Association<Customer, CustomerId>` and replace the assignment with a call to `Association.forId(…)`.
. Execute `PersistentModelTests` to verify everything still works as expected.
